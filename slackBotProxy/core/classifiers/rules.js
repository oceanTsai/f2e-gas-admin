...
// 寫成「修飾詞（可有可無）＋ 核心詞（必須）＋ 尾綴（可有可無）」而不是列舉整句：
// 實測「查詢剩餘額度」被漏掉，因為原本的樣式假設句子以核心詞開頭，接不住動詞前綴。
// 列舉整句的維護方式註定一直漏——每個人問法都不同，而漏掉的症狀是「打了沒反應」。
const RE_PURE_USAGE = new RegExp(
  '^\\s*'
  + '(?:請|麻煩|幫我|幫忙|我要|我想|想|要)*\\s*'          // 客套與意願
  + '(?:查詢|查查|查一下|查|看一下|看看|看|報|問)*\\s*'    // 動詞
  + '(?:一下)?\\s*'
  + '(?:claude|cc)?\\s*(?:的)?\\s*'
  + '(?:目前|現在|今天|本週|這週)*\\s*'                   // 時間修飾
  + '(?:剩餘|剩下|還剩|還有|剩)*\\s*(?:多少)?\\s*(?:的)?\\s*'
  + '(?:額度|用量|使用量|扣打|殘量|配額|quota|usage)\\s*'  // ← 核心詞，必須出現
  + '(?:還剩多少|剩多少|還有多少|剩下多少|用了多少|多少|如何|怎樣|怎麼樣|狀況|情況)?\\s*'
  + '(?:了|嗎|呢|喔|吧|啊)*\\s*'
  + '[?？!！。]*$',
  'i');
