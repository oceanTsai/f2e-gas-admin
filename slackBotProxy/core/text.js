...
//
//  為什麼需要這支：core/classifiers/shadowIntent.js 會把使用者那句話送到
//  **公司外的** LLM API（打哪一家見 core/llm/）。使用者貼上來問「這段在幹嘛」
//  的程式碼常常帶著業務邏輯或內部命名，不能照樣送出去，只留自然語言的意圖描述
//  就夠了（分類器只需要知道「他在問程式碼」，不需要知道程式碼寫了什麼）。
//  免費的 Gemini key 更會直接把輸入拿去訓練——但脫敏對所有 provider 一視同仁，
//  不是只有免費那家才做。
//
